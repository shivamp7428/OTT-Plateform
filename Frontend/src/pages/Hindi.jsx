...
const Hindi = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/api/v1/MovieRoute/hindi")
      .then((res) => res.json())
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4 text-center">Hindi Movie</h1>
      
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
        {movies.map((movie, index) => (
          <div key={index} className=" relative group border border-gray-800 rounded shadow ">
            <img
              src={movie.image?.url}
              alt={movie.title}
